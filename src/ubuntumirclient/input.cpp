...
void UbuntuInput::customEvent(QEvent* event)
{
    ...
    case mir_event_type_resize:
    {
        ...
        auto width = mir_resize_event_get_width(resizeEvent);
        auto height = mir_resize_event_get_height(resizeEvent);
        
        mIntegration->screen()->handleWindowSurfaceResize(width, height);
        ubuntuEvent->window->handleSurfaceResize(width, height);
        break;
    }
    case mir_event_type_surface:
    {
